...
namespace Company.API.Extensions
{
    public static class HttpExtensions
    {
        ...
        public static async Task<IResult> HttpSingleAsync<TEntity, TDto>(this IDbService db, int id) where TEntity : class, IEntity
            where TDto : class
        {
            var result = await db.SingleAsync<TEntity, TDto>(e =>
            e.Id.Equals(id));
        ...
        public static async Task<IResult> HttpPutAsync<TEntity, TDto>(this IDbService db, int id, TDto dto) where TEntity : class, IEntity
         where TDto : class
        {
            try
            {
            ...
            where TEntity : class, IReferenceEntity where TDto : class
        {
            try
            {
            var entity = db.AddAsync<TEntity, TDto>(dto);
            if (await db.SaveChangesAsync()) Results.NoContent();
            }
            catch (Exception ex)
            {
                return Results.BadRequest($"Couldn't delete the {typeof(TEntity).Name}entity.\n{ex}.");
